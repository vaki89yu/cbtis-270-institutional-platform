...
export async function getCurrentUser(): Promise<SessionUser | null> {
  ...
  // Primero usamos la sesión firmada. Así los módulos no dependen de que
  // Postgres responda perfectamente en cada navegación.
  ...
  if (hint && token) {
    try {
      ...
      // Continúa con la sesión persistida.
    }
  }
  ...
  try {
    ...
    // Sin acceso a BD no se puede validar la sesión persistida.
  }

  return null;
}port { createHmac, randomBytes, scryptSync, timingSafeEqual } from "node:crypto";
import { cookies } from "next/headers";
import { and, eq, gt } from "drizzle-orm";
import { db } from "@/db";
import { sessions, users, type User } from "@/db/schema";
