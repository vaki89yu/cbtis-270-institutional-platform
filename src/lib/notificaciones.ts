...
export async function registrarActividad(userId: number | null, accion: string, detalle?: string) {
  ...
  try {
    ...
    console.warn("No se pudo registrar la actividad (no bloqueante):", (error as Error).message?.slice(0, 150));
  }
}
...
export async function notificarDocentesRegistroAlumno({
  ...
  try {
    ...
    console.warn("[notificarRegistro] Error no bloqueante:", (error as Error).message?.slice(0, 150));
  }
}
...
export async function notificarDocentesInicioSesion(alumnoId: number) {
  try {
    ...
    console.warn("No se pudo notificar el inicio de sesión (no bloqueante):", (error as Error).message?.slice(0, 150));
  }
}
