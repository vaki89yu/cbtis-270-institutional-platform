...
async function safeQuery<T>(fn: () => Promise<T>, fallback: T): Promise<T> {
  try {
    ...
    console.warn("[consultas] DB query falló, fallback:", (error as Error).message?.slice(0, 150));
    return fallback;
  }
}
...
export async function resumenAsistenciaEstudiante(studentId: number) {
  return safeQuery(
    ...
    { total: 0, presentes: 0, retardos: 0, faltas: 0, justificados: 0, porcentaje: 100 },
  );
}
